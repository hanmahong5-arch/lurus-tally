...
// DeleteUseCase soft-deletes a supplier.
...
// Execute soft-deletes the supplier. Returns ErrNotFound if absent or already deleted.
func (uc *DeleteUseCase) Execute(ctx context.Context, tenantID, id uuid.UUID) error {
	if err := uc.repo.Delete(ctx, tenantID, id); err != nil {
		...
		return fmt.Errorf("supplier delete: %w", err)
	}
	return nil
}
