...
// the real RecordMovementUseCase + onboarding repo — the inline equivalent of
// the handler's stockAdapter, so this test exercises the same path the HTTP flow
// uses (sale recorded as a bill + its linked 'out' movement).
type seedStockAdapter struct {
	uc   *appstock.RecordMovementUseCase
	repo *repoonboarding.Repo
}
...
func (a *seedStockAdapter) RecordSale(ctx context.Context, req appob.DemoSaleRequest) error {
	billItemID, err := a.repo.InsertDemoSale(ctx, appob.DemoSaleBill{
		TenantID:    req.TenantID,
		ProductID:   req.ProductID,
		WarehouseID: req.WarehouseID,
		Qty:         req.Qty,
...
func TestOnboardingSeedVelocity_RetailLightsUp(t *testing.T) {
	...
	adapter := &seedStockAdapter{uc: recordMovementUC, repo: repoonboarding.New(db)}
	...
	// ── Reports: sales-top / gross-margin light up from the seeded sale bills ─
	repRows, err := reportsrepo.New(db).ListRecentSaleLines(ctx, tenantID, 30)
	if err != nil {
		t.Fatalf("list recent sale lines: %v", err)
	}
	soldProducts := map[string]bool{}
	for _, r := range repRows {
		if !r.Revenue.IsPositive() {
			t.Errorf("sale line %s: revenue must be > 0, got %s", r.ProductName, r.Revenue)
		}
		if !r.Margin.IsPositive() {
			t.Errorf("sale line %s: margin must be > 0 (sale price > cost), got %s", r.ProductName, r.Margin)
		}
		soldProducts[r.ProductName] = true
	}
	if len(soldProducts) != 3 {
		t.Errorf("reports: want all 3 demo products with sales, got %d distinct: %v", len(soldProducts), soldProducts)
	}

	// ── Honesty: each demo sale is one bill + one linked 'out' movement (1:1) ─
	var demoBills, demoOutMoves int
	if err := db.QueryRowContext(ctx,
	...
	if err := db.QueryRowContext(ctx,
		`SELECT count(*) FROM tally.stock_movement WHERE tenant_id=$1 AND direction='out'`, tenantID,
	).Scan(&demoOutMoves); err != nil {
		t.Fatalf("count out movements: %v", err)
	}
	if demoBills == 0 || demoBills != demoOutMoves {
		t.Errorf("demo sale bills (%d) must be 1:1 with 'out' movements (%d)", demoBills, demoOutMoves)
	}

	t.Logf("PASS: seed retail lit up — low-stock=1, suggestions=3, digest count=%d amount=%s, reports=3 products, %d sale bills",
		sum.ReplenishCount, sum.ReplenishAmountCNY, demoBills)
}
