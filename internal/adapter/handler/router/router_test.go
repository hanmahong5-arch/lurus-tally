...
func newTestRouter() *gin.Engine {
	h := health.New("dev")
	return router.New(h, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
}
...
// TestRouter_RegistersProjectRoutes verifies all project routes are registered (Story 28.2).
func TestRouter_RegistersProjectRoutes(t *testing.T) {
	r := newTestRouter()
	...
	routes := []struct {
		...
		{http.MethodGet, "/api/v1/projects/some-id"},
		{http.MethodPut, "/api/v1/projects/some-id"},
		{http.MethodDelete, "/api/v1/projects/some-id"},
		{http.MethodPost, "/api/v1/projects/some-id/restore"},
	}
	...
	for _, tc := range routes {
		...
		r.ServeHTTP(w, req)
		if w.Code == http.StatusNotFound {
			t.Errorf("%s %s returned 404 — project route not registered", tc.method, tc.path)
		}
	}
}
