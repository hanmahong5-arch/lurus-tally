...
--   * account_audit_log — write-only flow of business-significant events
--                     (PAT created / revoked, bill approved, etc.) for the
--                     "活动日志" tab. The account_ prefix is deliberate:
--                     tally.audit_log already exists (migration 000009 — a
--                     dormant jshERP-derived CRUD trail with a different
--                     column shape), so a fresh table avoids the collision.
...
CREATE TABLE IF NOT EXISTS tally.account_audit_log (
    id           UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id    UUID         NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_account_audit_log_tenant_created
    ON tally.account_audit_log (tenant_id, created_at DESC);

ALTER TABLE tally.account_audit_log ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'tally' AND tablename = 'account_audit_log' AND policyname = 'tenant_isolation'
    ) THEN
        CREATE POLICY tenant_isolation ON tally.account_audit_log
            USING (tenant_id = current_setting('app.tenant_id')::uuid);
    END IF;
END
$$;
