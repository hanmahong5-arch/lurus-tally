# Tally (2b-svc-psi) CLAUDE.md Archive — 2026-06-10

> 来源: `2b-svc-psi/CLAUDE.md` 瘦身搬出的策略/路线图小节。真源仍以 `./_bmad-output/planning-artifacts/` 为准;本文件保留历史叙事供回溯,不作运行决策依据。
...
## Current State — full Wave 3 / STAGE narrative (snapshot 2026-06-04)

🚧 Epic 1 closed + Billing integration (Story 10.1) + 进销存核心 vertical slice 闭环 (Epic 4/5/6/7 后端齐 + Stock REST + 库存 UI). Images: `ghcr.io/hanmahong5-arch/lurus-tally-{backend,web}`. Sprint 详情 → `./_bmad-output/planning-artifacts/sprint-status.yaml`.

**RLS backstop (Wave 3, 2026-06-04)**: FORCE + 逐请求 pin (`middleware.TenantDB` → `dbscope`) 全表生效;Phase-3 strict-flip(空 GUC→fail-closed)已覆盖**所有租户表**(000045/046/047),仅 webhook/background/pre-tenant-auth 表按设计放宽。000048 关掉 unit_def 的 is_system 跨租户写洞;`dbscope` 加并发探测器(pinned conn 重叠使用即结构化告警)。证明见 `tests/integration/rls_*`(非-superuser owner e2e + StrictFlip + RLSPolicyShape)。
