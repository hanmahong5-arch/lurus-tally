...
func newClient(t *testing.T, srv *httptest.Server) *platform.Client {
	t.Helper()
...
func TestNew_RejectsEmptyConfig(t *testing.T) {
	if _, err := platform.New(platform.Config{APIKey: "k"}); err == nil {
		t.Error("expected error when BaseURL empty")
	}
	if _, err := platform.New(platform.Config{BaseURL: "http://x"}); err == nil {
...
func TestSubscriptionCheckout_WalletActivation(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var body platform.SubscriptionCheckoutRequest
	...
	resp, err := newClient(t, srv).SubscriptionCheckout(context.Background(), platform.SubscriptionCheckoutRequest{
		AccountID:     42,
...
func TestSubscriptionCheckout_PlatformDown_MapsToUnavailable(t *testing.T) {
	...
	if !platform.IsCode(err, platform.ErrCodeUnavailable) {
