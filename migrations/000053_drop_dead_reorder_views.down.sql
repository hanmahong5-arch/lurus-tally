-- 000053_drop_dead_reorder_views.down.sql
-- Restore exactly what the up migration removed, reproducing the relevant parts
-- of migrations 000005 (columns) and 000011 (views). Columns are re-added first
-- because the views reference them.

ALTER TABLE tally.product ADD COLUMN IF NOT EXISTS predicted_monthly_demand NUMERIC(18,4);
ALTER TABLE tally.product ADD COLUMN IF NOT EXISTS predicted_stockout_at    TIMESTAMPTZ;
ALTER TABLE tally.product ADD COLUMN IF NOT EXISTS recommendation_notes     TEXT;
